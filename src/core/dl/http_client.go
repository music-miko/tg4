...
// maskSensitiveInfo removes API keys from error messages.
func maskSensitiveInfo(msg string) string {
	if config.ArcApiKey != "" {
		msg = strings.ReplaceAll(msg, config.ArcApiKey, "REDACTED")
	}
	if config.FallenApiKey != "" {
		msg = strings.ReplaceAll(msg, config.FallenApiKey, "REDACTED")
	}
	return msg
}
