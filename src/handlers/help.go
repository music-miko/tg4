...
func helpCallbackHandler(c *td.Client, cb *td.UpdateNewCallbackQuery) error {
	...
	if strings.Contains(data, "setup_guide_helper") {
		...
		guide := fmt.Sprintf(
			"🅰️ <b>Setup Guide</b>\n\nGet %s up and running in your group in under a minute:\n\n"+
				...
				"<b>Step 2 — Promote the bot</b>\nMake %s an admin and grant the <b>Invite Users via Link</b> permission. This is required for voice chats.\n\n"+
		...
		_, _ = cb.EditMessageCaption(c, guide, &td.EditCaptionOpts{ReplyMarkup: core.SetupGuideKeyboard(), ParseMode: "HTML"})
		return nil
	}
