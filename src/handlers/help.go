...
func helpCallbackHandler(c *td.Client, cb *td.UpdateNewCallbackQuery) error {
	...
	if strings.Contains(data, "guide_1") {
		_ = cb.Answer(c, 0, false, "Setup Guide", "")
		guideText := fmt.Sprintf(
			"<emoji id='5769403725898584391'>🅰️</emoji><b> Setup Guide</b>\n\n"+
		...
		_, _ = cb.EditMessageCaption(c, guideText, &td.EditCaptionOpts{ReplyMarkup: core.GuideBackKeyboard(), ParseMode: "HTML"})
		return nil
	}
