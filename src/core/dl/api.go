...
// For non-YouTube platforms (Spotify, Apple, SoundCloud, etc.) we use ApiUrl2/ApiKey2
// (the secondary API) so that ApiUrl/ApiKey is reserved for the Arc Music API
// (YouTube job-based downloads).
func newApiData(query string) *apiData {
	apiURL := strings.TrimRight(config.ApiUrl2, "/")
	apiKey := config.ApiKey2
	// Fall back to primary API if secondary is not configured
	if apiURL == "" || apiKey == "" {
