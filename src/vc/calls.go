...
func (c *TelegramCalls) SeekStream(bot *td.Client, chatID int64, filePath string, toSeek, duration int, isVideo bool) error {
	...
	remaining := duration - toSeek
	if remaining <= 0 {
		return errors.New("seek position is beyond the end of the track")
	}

	// Always use "-ss <seek>" as a pre-input flag so ffmpeg seeks efficiently.
	// Use "-t <remaining>" (output duration) — NOT "-to" (absolute end timestamp)
	// so ntgcalls receives exactly the correct number of frames/samples and
	// fires OnStreamEnd at the right moment.
	//
	// Note: we never embed "-i <path>" here — getMediaDescription always adds it.
	ffmpegParams := fmt.Sprintf("-ss %d -t %d", toSeek, remaining)
