...
// Priority:
//  1. ArcMusic API (API_URL2 / API_KEY2) — includes DB channel cache check
//  2. Legacy API (API_URL / API_KEY) — audio only
//  3. yt-dlp fallback
func (y *youTubeData) downloadTrack(info utils.TrackInfo, video bool) (string, error) {
	// 1. ArcMusic API for both audio and video
	if ArcIsConfigured() {
		if filePath, err := ArcDownload(info.Id, video); err == nil {
			return filePath, nil
		}
		slog.Info("downloadTrack: ArcAPI failed, falling back", "video_id", info.Id)
	}

	// 2. Legacy API (audio only)
